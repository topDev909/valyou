	...
	public function loadmore(){
		...
		if(!empty($artists)){
			...
			foreach($artists as $key=>$artist)
			{
                ...
                <a href="artist/stock-price/'.$artist->id.'">
				<img src="'.asset($artist->photo).'"
				width="50" height="50" class="rounded-circle avatar-xm m-1 artist-list-photo" alt="">
                </a>
				</div>
				<div class="col-lg-8 col-8 d-flex flex-column justify-content-center">
				<a style="display:block" href="artist/stock-price/'.$artist->id.'">
				<p class="uploading-section-artist-p1">'.$artist->brand.'</p>
				</a>
				<p class="uploading-section-artist-p2">'.$stock_value;

				if((!empty($artist->change_stock)) && $artist->change_stock > 0){
					$html .='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="text-green d-inline-block"><img class="uploading-section-artist-icon-img"
					src="'.asset('assets/images/valyoux/green_arrow_price_going_up.svg').'">
					+ '.numberformat($artist->change_stock).'%</span>';
				}else{
					$html .='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span class="text-danger d-inline-block"><img class="uploading-section-artist-icon-img"
					src="'.asset('assets/images/valyoux/pink_arrow_circle_down.svg').'">
					'.signFormat($artist->change_stock,0).'% </span>';
				}
				$html .='</p>
				</div>
				</div>
			...
			foreach($artists as $key2=>$artist)
			{
                ...
                <a href="artist/stock-price/'.$artist->id.'" style="padding:0px;">
				<img src="'.asset($artist->photo).'" width="59" height="59"
				...
				<div class="ml-s-5">
                <a href="artist/stock-price/'.$artist->id.'" style="padding:0px;">
				<p class="uploading-section-artist-mobile-p1">'.$artist->brand.'</p>
                </a>
				</div>
				<div class="ml-s-5">
				<p class="uploading-section-artist-mobile-p2">'.$stock_value.'</p>
				</div>
				<div class="ml-s-5">
				<p class="uploading-section-artist-mobile-p3">';

				if((!empty($artist->change_stock)) && $artist->change_stock > 0){
					$html2 .='<img class="uploading-section-artist-mobile--icon-img"
					src="'.asset('assets/images/valyoux/green_arrow_price_going_up.svg').'">&nbsp;
					<span class="">+'.numberformat($artist->change_stock).'% </span>';
				}else{
					$html2 .='    <img class="uploading-section-artist-mobile--icon-img"
					src="'.asset('assets/images/valyoux/pink_arrow_circle_down.svg').'">&nbsp;
					<span class="">'.signFormat($artist->change_stock,0).'% </span>';
				}
				$html2 .='</p>
				</div>
				</div>
